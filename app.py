import os
# Create the final Streamlit app code with all fixes and features
...
if uploaded_file:
    ...
    st.markdown(f"#### 📏 Original Image Size: `{original_size[0]}×{original_size[1]}` → Resized to `{IMG_SIZE[0]}×{IMG_SIZE[1]}`")
    ...
    try:
        img_batch = tf.expand_dims(tf.keras.preprocessing.image.img_to_array(img) / 255.0, axis=0)
        heatmap = make_gradcam_heatmap(img_batch, model, last_conv_layer_name="Conv_1")
        result = overlay_heatmap(img, heatmap)
        st.image(result, caption="Grad-CAM Overlay", use_container_width=True)
    ...
    log_df = pd.DataFrame([[uploaded_file.name, label, round(prob, 2)]], columns=["filename", "label", "probability"])
